import json
from flask import Flask, flash, session, render_template, redirect, request, abort, url_for, escape, jsonify
...
@app.route("/account/recoveryupdate", methods=['POST'])
@login_required
...
def signup():
	...
	if request.method == 'GET':
		return render_template("signup.html", session=session, config=app.config)
...
def pickcharactername(name):
	...
	if len(possiblecharacters)>0:
		...
		checkmask = None
		if session["accountStatus"] == "Internal":
			checkmask = app.config["auth"]["keymasks"]["internal"]
		if session["accountStatus"] == "Ally":
			checkmask = app.config["auth"]["keymasks"]["ally"]
		if checkmask:
			if not keytools.isKeyThisMask(session["keyid"], session["vcode"], checkmask):
				flash("Your key is not the correct mask, please start again.", "danger")
...
@app.route("/apiv1/ping/<string:group>", methods=["POST",])
@api_key_required
def apiping(group):
	count = pingbot.groupbroadcast(request.form["sender"], "(|(authGroup={0})(corporation={0})(alliance={0}))".format(group), request.form["message"], group)
	return jsonify({"group":group, "message":request.form["message"], "sender": request.form["sender"], "count": count, "status": "okay" if count>0 else "not okay"})
